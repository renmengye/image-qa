...
with open('../data/sentiment/rt-polarity.pos') as f:
    lines = f.readlines()

pos_marker = len(lines)
with open('../data/sentiment/rt-polarity.neg') as f:
...
with open('../data/sentiment/word_freq.txt', 'w+') as f:
    for k in sorted_key:
...
train_data = np.array((input_, target_, 0), object)

# Output vocab file for converting word2vec
with open('../data/sentiment/vocabs.txt', 'w+') as vocab_f:
