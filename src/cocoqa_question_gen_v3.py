...
parseFilename = '%s/captions.parse.txt' % (folder)
imgidsFilename = '%s/imgids.txt' % (folder)
outputFilename = '%s/qa.pkl' % (folder)
...
class QuestionGenerator:
    ...
    def askWhere(self, root):
        ...
        def traverse(node, parent):
            ...
            #print node.className, node.text
            cont = True
            ...
            if len(node.children) > 1 and \
                node.children[1].className == 'VP':
                    ...
                    while(len(c.children) > 0):
                        c = c.children[0]
                    if c.text.startswith('sink') or \
                        c.text.startswith('counter') or \
                        c.text.startswith('cupboard') or \
                        c.text == 'has' or \
                        c.text == 'have' or \
                        c.text.startswith('contain') or \
                        c.text == 'can':
                        cont = False
            ...
            if node.className == 'PP' and \
                ...
                if c.className == 'NN'and \
                    ...
                    not c.text.startswith('shirt') and \
                    not c.text.startswith('uniform') and \
                    not c.text.startswith('jacket') and \
                    not c.text.startswith('dress') and \
                    not c.text.startswith('hat') and \
                    not c.text.startswith('tie') and \
                    not c.text.startswith('enclosure') and \
                    not c.text.startswith('suit') and \
                    not c.text.startswith('photo') and \
                    not c.text.startswith('picture') and \
                    not c.text.startswith('round'):
                    found[0] = True
                    answer[0] = c.text 
    ...
    def askWhoWhat(self, root):
        ...
        def traverse(node, parent):
            ...
            if node.className == 'PP' and \
                (node.children[0].text == 'with' or \
                node.children[0].text == 'of' or \
                node.children[0].text == 'in' or \
                node.children[0].text == 'down' or \
                node.children[0].text == 'as'):
                cont = False
            ...
            if len(node.children) > 1 and \
                node.children[1].className == 'VP':
                    ...
                    if c.text.startswith('sink') or \
                        ...
                        c.text == 'have' or \
                        c.text.startswith('contain') or \
                        c.text.startswith('spaniel') or \
                        c.text.startswith('mirror') or \
                        c.text.startswith('shower') or \
                        c.text.startswith('stove') or \
                        c.text.startswith('bowl') or \
                        c.text.startswith('tile') or \
                        c.text.startswith('mouthwash'):
                        cont = False
                ...
                for child in node.children:
                    ...
                    if parent is not None:
                        if node.children.index(child) == len(node.children) - 1:
                            if parent.children.index(node) != len(parent.children) - 1:
                                if parent.children[parent.children.index(node) + 1].className == 'NP':
                                    break
                        # The two people are walking down the ``beach''
                        foundDown = False
                        if parent.children.index(node) != 0:
                            for sib in parent.children[parent.children.index(node) - 1].children:
                                #print sib.className, sib.text
                                if sib.text == 'down':
                                    foundDown = True
                        if foundDown:
                            break
                    if child.className == 'NN' or child.className == 'NNS':
                        ...
                        if lexname is not None:
                            # IGNORE ALL WHO QUESTIONS!!!
                            # if lexname == 'noun.person':
                            #     whword = 'who'
                            #lexname == 'noun.body' or \
                            if (lexname == 'noun.animal' or \
                                lexname == 'noun.artifact' or \
                                lexname == 'noun.food' or \
                                lexname == 'noun.object' or \
                                lexname == 'noun.plant' or \
                                lexname == 'noun.possession' or \
                                lexname == 'noun.shape') and (
                                not child.text.startswith('female') and \
                                not child.text.startswith('doll') and \
                                not child.text.startswith('ride') and \
                                not child.text.startswith('past') and \
                                not child.text.startswith('teddy') and \
                                not child.text.startswith('fan') and \
                                not child.text == 'arm' and \
                                not child.text == 'arms' and \
                                not child.text.startswith('slope') and \
                                not child.text.startswith('shoot') and \
                                not child.text == 'photo' and \
                                not child.text == 'space'):
                                    whword = 'what'
                ...
                if replace != None:
                    ...
                    for child in node.children:
                        ...
                        if child != replace and (
                            ...
                            child.text == 'tennis' or \
                            child.text == 'delivery'):
                            #print 'removed', child.text
                            toremove.append(child)
                        #elif child != replace:
                            #print 'not removed', child.text, lexname
            ...
            if node.className == 'NP':
                for child in node.children:
                    ...
                    if (child.className == 'NN' or child.className == 'NNS') and \
                        not child.text.startswith('ride') and \
                        not child.text.startswith('past') and \
                        not child.text.startswith('standing') and \
                        not child.text.startswith('riding'):
                        obj[0] = child
...
def questionGen():
        ...
        for line in f:
            if len(parser.rootsList) > 0:
                ...
                for qaitem in gen.askWhoWhat(parser.rootsList[0].copy()):
                    if qaitem[0] == 'what ?' or qaitem[0] == 'who ?' or len(qaitem[0].split(' ')) < 5:
                        continue
                    else:
                        question = qaitem[0]
                    questionCount += 1
                    ...
                    questionAll.append((qaitem[0], qaitem[1], imgid, 0, originalSent))
                for qaitem in gen.askHowMany(parser.rootsList[0].copy()):
                    ...
                    questionAll.append((qaitem[0], qaitem[1], imgid, 1, originalSent))
                for qaitem in gen.askColor(parser.rootsList[0].copy()):
                    ...
                    questionAll.append((qaitem[0], qaitem[1], imgid, 2, originalSent))
                for qaitem in gen.askWhere(parser.rootsList[0].copy()):
                    questionCount += 1
                    questionWhereCount += 1
                    hasItem = True
                    print ('Question %d:' % questionCount), qaitem[0], 'Answer:', qaitem[1]
                    questionAll.append((qaitem[0], qaitem[1], imgid, 3, originalSent))
                if hasItem:
    ...
    #     f.write('#,question,answer,original,type\n')
    #     for i,item in enumerate(questionAll):
    #         question = item[0]
    #         f.write(str(i+1)+','+item[0].replace(',','')+','+item[1]+','+item[4].replace(',','')+','+str(item[3])+'\n')
...
if __name__ == '__main__':
    if len(sys.argv) > 2:
        if sys.argv[1] == '-t':
            ...
            gen = QuestionGenerator()
            print gen.lookupLexname(sys.argv[2])
    else:
        questionGen()
